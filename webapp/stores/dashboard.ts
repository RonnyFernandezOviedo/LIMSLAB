...
export const useDashBoardStore = defineStore('dashboard', () => {
    const dashboard = ref({
        currentTab: 'general',
        tabs: ['general', 'recursos', 'rezagado', 'desempeño', 'noticias',], // 'tat' 'line-listing'
        showFilters: false,
        filterRange: { from: '', fromIso: '', to: '', toIso: '' },
        currentFilter: 'TW',
        filters: ['T', 'Y', 'TW', 'LW', 'TM', 'LM', 'TY'], //'TQ', 'LQ',
        overViewStats: {
    ...
    const filterToolTip = (filter: string): string => {
        if (filter === 'T') return 'Hoy';
        if (filter === 'Y') return 'Ayer';
        if (filter === 'TW') return 'Esta semana';
        if (filter === 'LW') return 'Semana pasada';
        if (filter === 'TM') return 'Este mes';
        if (filter === 'LM') return 'Mes pasado';
        if (filter === 'TY') return 'Este Año';
