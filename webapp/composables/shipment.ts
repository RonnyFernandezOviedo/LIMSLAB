...
export default function useShipmentComposable() {
    ...
    const manageSamples = (shipmentUid, samplesMetadata, action="assign") => {
        try {
          Swal.fire({
            title: "Esta seguro?",
            text: `Usted quiere ${action} seleccionado?`,
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            confirmButtonText: `Si, ${action} ahora!`,
            cancelButtonText: "No, cancelar aplicacion!",
          }).then((result) => {
    ...
    const actionShipment = (uid, action="finalise") => {
      try {
        Swal.fire({
          title: "Esta seguro?",
          text: `Usted quiere ${action} el envio?`,
          icon: "warning",
          showCancelButton: true,
          confirmButtonColor: "#3085d6",
          cancelButtonColor: "#d33",
    ...
    const downloadManifest = async uid => {
      try {
          await Swal.fire({
              ...
              cancelButtonText: 'No, no decargar!',
          }).then(async result => {
              if (result.isConfirmed) {
