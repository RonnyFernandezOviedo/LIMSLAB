    ...
    <div class="flex-none relative">
      ...
      <nav id="aside-nav" class="" role="navigation">
        <router-link v-show="guards.canAccessPage(guards.pages.DASHBOARD)" to="/dashboard" id="dashboard-link"
          ...
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Panel</span>
          <span v-show="!viewNavText" class="tooltip">Panel</span>
        </router-link>
        <router-link v-show="guards.canAccessPage(guards.pages.PATIENTS_COMPACT)" to="/patients-compact"
          ...
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Resumen</span>
          <span v-show="!viewNavText" class="tooltip">Resumen</span>
        </router-link>
        <router-link v-show="guards.canAccessPage(guards.pages.PATIENTS)" to="/patients" id="patients-link"
          class="flex items-center has-tooltip mt-1 py-2 px-6 text-gray-100 hover:bg-gray-800 hover:bg-opacity-25 hover:text-white border-l-4 border-gray-800">
          <span class="mr-4"><font-awesome-icon icon="atom" /></span>
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Solicitud</span>
        <span v-show="!viewNavText" class="tooltip">Solicitud</span>
      </router-link>
      <router-link v-show="guards.canAccessPage(guards.pages.CLIENTS)" to="/clients" id="clients-link"
        class="flex items-center has-tooltip mt-1 py-2 px-6 text-gray-100 hover:bg-gray-800 hover:bg-opacity-25 hover:text-white border-l-4 border-gray-800">
        <span class="mr-4"><font-awesome-icon icon="contact-book" /></span>
        <span
          :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Clientes</span>
        <span v-show="!viewNavText" class="tooltip">Clientes</span>
      </router-link>
        <router-link v-show="guards.canAccessPage(guards.pages.SAMPLES)" to="/samples" id="samples-link"
          ...
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Muestras</span>
          <span v-show="!viewNavText" class="tooltip">Muestras</span>
        </router-link>
        <router-link v-show="guards.canAccessPage(guards.pages.WORKSHEETS)" to="/worksheets" id="worksheets-link"
          ...
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Hoja trabajo</span>
          <span v-show="!viewNavText" class="tooltip">Hoja trabajo</span>
        </router-link>
        <!-- <router-link
          v-show="guards.canAccessPage(guards.pages.QC_SAMPLES)"to="/quality-control"id="quality-control-link"
          class="flex items-center has-tooltip mt-1 py-2 px-6 text-gray-100 hover:bg-gray-800 hover:bg-opacity-25 hover:text-white border-l-4 border-gray-800">
        ...
        <router-link v-show="guards.canAccessPage(guards.pages.NOTICE_MANAGER)" to="/notice-manager"
          ...
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Notificaciones</span>
          <span v-show="!viewNavText" class="tooltip">Notificaciones</span>
        </router-link>
        ...
        <router-link v-show="guards.canAccessPage(guards.pages.INVENTORY)" to="/inventory" id="inventory-link"
          ...
          <span
            :class="[' transition-all duration-500', viewNavText ? 'scale-100' : 'scale-0 w-0 overflow-hidden']">Inventario</span>
          <span v-show="!viewNavText" class="tooltip">Inventario</span>
        </router-link>
      </nav>
    </div>
