...
<template>
  <LoadingMessage v-if="loadongMeta" :message="'Cargando su reporte...'" />
  <section v-else>
    <div v-if="impressMeta.length > 0" class="flex justify-start mt-4 mr-4">
          ...
          <div class="flex justify-between items-center space-x-4">
            <div class="flex-1 min-w-0">
              ...
              <p class="text-sm text-gray-500 truncat">
                Generado en {{ report.dateGenerated }}
              </p>
      ...
      <div class="ml-4 w-full">
        <h5>Json Impresion</h5>
        <hr class="mb-2" />
        <JsonPreViewer :data="selectedMeta" :wrapper="'max-h-96 overflow-y-scroll'" />
      </div>
    </div>
    <div v-else class="mt-4 text-red-500">Esta muestra no tiene reportes para imprimir</div>
  </section>
</template>
