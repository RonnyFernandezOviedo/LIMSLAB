...
<template>
  ...
  <section class="col-span-12 mt-2">
    ...
    <div class="pt-4" v-motion :initial="{ opacity: 0, y: 100 }" :enter="{ opacity: 1, y: 0, scale: 1 }"
      ...
      <tab-overview v-if="dashboard.currentTab === 'general'" />
      <tab-resource v-if="dashboard.currentTab === 'recursos'" />
      <tab-laggard v-if="dashboard.currentTab === 'rezagado'" />
      <tab-peformance v-if="dashboard.currentTab === 'desempeño'" />
      <tab-tat v-if="dashboard.currentTab === 'tat'" />
      <tab-notice v-if="dashboard.currentTab === 'noticias'" />
      <tab-line-listing v-if="dashboard.currentTab === 'xxxxxline-listing'" /> <!--ronny remve-->
    </div>
  </section>
</template>
