...
function FormManager(create: boolean, obj = {} as IIdentification): void {
  ...
  formTitle.value = (create ? 'CREAR' : 'EDITAR') + ' ' + "IDENTIFICACION PERSONAL";
  if (create) {
...
<template>
  <div class="container w-full my-4">
    ...
    <button @click="FormManager(true)"
      class="px-2 py-1 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Agregar Identificacion</button>
    <hr>
            ...
            <tr v-for="identification in identifications" :key="identification?.uid">
              ...
              <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                <button @click="FormManager(false, identification)"
                  class="px-2 py-1 mr-2 border-sky-800 border text-sky-800 rounded-sm transition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Editar</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <form action="post" class="p-1">
        <div class="grid grid-cols-2 gap-x-4 mb-4">
          <label class="flex items-center gap-x-4 whitespace-nowrap col-span-2 mb-2">
            <span class="text-gray-700">Nombre identificacion</span>
