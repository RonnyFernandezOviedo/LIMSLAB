<script setup lang="ts">
  ...
  function FormManager(create: boolean, obj = {} as IManufacturer):void {
    ...
    formTitle.value = (create ? 'CREAR' : 'EDITAR') + ' ' + "FABRICANTE";
    if (create) {
    ...
    <div class="container w-full my-4">
        <hr>
          <button @click="FormManager(true)"
           class="px-2 py-1 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Agregar Fabricante</button>
        <hr>
            ...
            <table class="min-w-full">
                ...
                <tr v-for="manufacturer in manufacturers" :key="manufacturer?.uid">
                    ...
                    <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                        <button @click="FormManager(false, manufacturer)" class="px-2 py-1 mr-2 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Editar</button>
                    </td>
                </tr>
                </tbody>
            </table>
