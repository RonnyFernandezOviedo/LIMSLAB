<script setup lang="ts">
  ...
  function FormManager(create: boolean, obj: IQCLevel = {}):void {
    ...
    formTitle.value = (create ? 'CREAR' : 'EDITAR') + ' ' + "RANGOS QC";
    if (create) {
    ...
    <div class="container w-full my-4">
        <hr>
          <button @click="FormManager(true)"
           class="px-2 py-1 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Agregar rango QC</button>
        <hr>
        ...
        <div class="overflow-x-auto mt-4">
            ...
            <table class="min-w-full">
                <thead>
                <tr>
                    <th class="px-1 py-1 border-b-2 border-gray-300 text-left text-sm leading-4 text-gray-800 tracking-wider">Rango</th>
                ...
                <tr v-for="level in qcLevels"  :key="level?.uid">
                    ...
                    <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                        <button @click="FormManager(false, level)" class="px-2 py-1 mr-2 border-sky-800 border text-sky-800 rounded-sm transition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Editar</button>
                    </td>
                </tr>
                </tbody>
            </table>
      ...
      <form action="post" class="p-1">
        <div class="grid grid-cols-2 gap-x-4 mb-4">
          <label class="block col-span-2 mb-2">
            <span class="text-gray-700">Rango QC</span>
            <input
