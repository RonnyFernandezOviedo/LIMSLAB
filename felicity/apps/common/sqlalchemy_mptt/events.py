...
def mptt_before_insert(mapper, connection, instance):
    ...
    if instance.parent_id is None:
        ...
        (
            ...
            select(table.c.lft, table.c.rgt, table.c.tree_id, table.c.level).where(
...
def mptt_before_delete(mapper, connection, instance, delete=True):
    ...
    if instance.parent_id is not None or not delete:
        ...
        connection.execute(
            table.update()
            .where(and_(table.c.rgt > rgt, table.c.tree_id == tree_id))
            .values(
                lft=case((table.c.lft > lft, table.c.lft - delta), else_=table.c.lft),
                rgt=case((table.c.rgt >= rgt, table.c.rgt - delta), else_=table.c.rgt),
            )
        )
...
def mptt_before_update(mapper, connection, instance):
    ...
    (
        ...
        select(
            table.c.lft, table.c.rgt, table.c.tree_id, table.c.parent_id, table.c.level
