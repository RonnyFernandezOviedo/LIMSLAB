...
class PatientMutations:
    ...
    async def create_patient(self, info, payload: PatientInputType) -> PatientResponse:
        ...
        if not client:
            return OperationError(
                error=f"Cliente con uid {payload.client_uid} no existe"
            )
    ...
    async def update_patient(
        ...
        if not uid:
            return OperationError(error="No se proporcionó uid para la actualización de identidad obj")

        patient: models.Patient = await models.Patient.get(uid=uid)
        if not patient:
            return OperationError(
                error=f"solicitud con uid {uid} no econtrado. No se puede actualizar obj ..."
            )
